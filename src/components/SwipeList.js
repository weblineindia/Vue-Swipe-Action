/****************************************************************
  ...
  This File will be used for swipe list card actions
...
export default {
  ...
  methods: {
    revealRight(index) {
      if (!this.$refs.items[index]) return;
    ...
    close(index) {
      ...
      if (index === undefined) return this.$refs.items.forEach(i => i.close());
    ...
    _onReveal(item, index, event) {
      ...
      this._emitRevealed(
        Object.assign(this.innerRevealed, { [index]: event.side })
      );
    },
    _onClose(item, index) {
      this.$emit("closed", {
      ...
      const newRevealed = Object.assign({}, this.innerRevealed);
