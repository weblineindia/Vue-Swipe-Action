/****************************************************************
  ...
  This File will be used for swipe out actions
...
export default {
  ...
  methods: {
    ...
    _swipeListener({ offset }) {
      ...
      if (!this.$scopedSlots.left && newX > 0) return this._animateSlide(0);

      if (!this.$scopedSlots.right && newX < 0) return this._animateSlide(0);
    ...
    _reveal(dir, recalculateWidth) {
      ...
      if (dir && !this.$refs[dir]) dir = false;
    ...
    _shiftLeftActions(newX) {
      if (!this.$scopedSlots.left) return;
    ...
    _shiftRightActions(newX) {
      if (!this.$scopedSlots.right) return;
